...
const zh = {
  loading: '正在加载…', badgeCompatibility: '兼容性提示：官方 Harness 尚无逐工具徽章与设置导航图标插槽。设置与审查可用；命令右侧审查/拒绝图标和设置选项卡 SVG 需要配套的 tool.call.badges 与 settings.section.icon 补丁，未安装时保留原生显示。',
...
export function AutoReviewCallBadge({ callId, useReviewStatus, t }: BadgeProps): ReactNode {
...
function AutoReviewSettingsSection({ read, update, reset: resetSettings, metrics: readMetrics, t }: Props): ReactNode {
  ...
  if (snapshot.status !== 'ready' || draft === undefined) {
    return <section className="ar-page" aria-busy="true"><p className="ar-muted">{snapshot.status === 'unavailable' ? t('failed') : t('loading')}</p></section>
  }
  ...
  return (
    <section className="ar-page">
      ...
      <p className="ar-muted" data-auto-review-badge-compatibility="requires-owner-slot">{t('badgeCompatibility')}</p>
...
export function apply(ctx: ClientContext): void {
  ...
  badgeSlots.inject('tool.call.badges', () => badgeSlots.register({
    ...
    inject: (sessionId): AutoReviewBadgeInjected => ({ hooks: { reviewStatus: reviewStatus.source(sessionId) } }),
  }, AutoReviewCallBadge))
}
