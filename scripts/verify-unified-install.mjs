...
const [artifactArg,hostArg,expected,previousArtifact]=process.argv.slice(2)
...
if (previousArtifact) {
  run(['plugin','--profile','web','add',resolve(previousArtifact),'--registry=https://registry.npmjs.org/'])
  writeFileSync(join(root,'preserve-test-marker'), 'not-plugin-owned')
}
...
for (const channel of ['rc6','rc2','alpha5']) assert(!existsSync(join(plugin,'ui',channel)), 'unused UI cohort left in package')
const profile = JSON.parse(readFileSync(join(root,'profiles/web/package.json'),'utf8'))
assert.equal(profile.dsh.profile.bundles.filter(name => name === '@jhckevin/dsh-auto-review').length, 1, 'duplicate active plugin')
if (previousArtifact) assert.equal(readFileSync(join(root,'preserve-test-marker'),'utf8'), 'not-plugin-owned')
process.argv[1]=cli
...
const {prepareUi: installUi}=await import(pathToFileURL(join(plugin,'scripts/prepare-ui.mjs')))
assert.equal((await installUi(uiRoot,{check:true})).version,expected)
assert.equal((await installUi(uiRoot)).changed,2)
assert.equal((await installUi(uiRoot)).changed,0)
assert.equal((await installUi(uiRoot,{restore:true})).changed,2)
