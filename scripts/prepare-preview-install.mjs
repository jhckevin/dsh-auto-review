...
for (const { manifest } of packages) if (channelRuntime[manifest.name]) assert.equal(manifest.version, channelRuntime[manifest.name], 'unvalidated runtime version for DSH channel')
