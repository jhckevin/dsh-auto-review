...
## 0.5.7-alpha.1 - 2026-09-03

目标 DSH：0.1.2-alpha.5；独立兼容分支，未部署线上，未发布 npm。

- 迁移原生 Session 快照/位置读取、ToolCallId、JSON 类型与 settings namespace，移除旧 client-runtime 依赖。
- 加入 sessionProjections 装配与策略激活检查，新增 never 禁止审批的零执行回归。
- 新版设置页面使用 Cordis Context 与框架 observable hooks；配套独立原生槽位补丁保留工具徽章和设置导航 SVG。
- 兼容门禁检查实际安装版本与真实服务链；完整 Guardian、模型 API 与生产 WebUI 验收不在本次结论内。
