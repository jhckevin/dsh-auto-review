...
## 0.6.2-rc.1 — 2026-09-06

- 启动及付费 Reviewer 调用前检查原生桥接，不可用时不创建付费调用。
- 常规与升级模型共享尝试预算；失败、超时和处置尾包进入统一用量账本。
- WebUI 根据真实插槽声明提示适配状态，展示缓存、未缓存输入和未知用量。
- 三宿主矩阵新增真实 Loop 取消、三拒绝熔断与下一回合恢复测试。
- 同步真实账本截图和 next 通道安装说明；仍为候选版，不宣称完整生产或自主拒绝矩阵已验收。

## 0.6.1 — release preparation

- 修复设置页底栏遮挡模型输入框，更新真实宿主截图。
- 统一包只按需下载匹配 DSH 的 UI 适配，增加哈希、下载大小、镜像与离线还原检查。
- 使用官方同名包升级替换旧版本；不再打包未选择的两套界面。
- README 增加简明审查流程与额外 token 成本提醒。

## 0.6.0 — release preparation

- 一个包覆盖三种已验证 DSH 宿主，补齐 rc6、rc2、alpha5 的原生工具盾牌与设置图标。
- UI 安装器按宿主版本自动选择，拒绝未知版本、文件篡改和混装，支持备份、幂等安装与还原。
- Actions 宿主矩阵增加实际包内 UI 安装与还原检查；补充 npm 检索关键词和简洁图文安装说明。

## 0.6.0-beta.2 — candidate

- 精简设置页，默认折叠统计与高级说明，修正开关对比度和导航 SVG。
- 拒绝熔断调用原生 turn cancellation；新增 WebUI 与可选终端提示，防止旧回合和并发晚到结果继续执行。
- 修正 reviewer 历史裁剪误收 reasoning，以及工具密集时丢失当前用户要求的问题。
- 随包提供 rc2 原生 UI owner 与完整性校验、备份、还原安装器；保留三宿主核心适配边界。
- 更新真实截图、安装步骤和成本分析；未切换默认 npm 通道。

## 0.5.6-rc.4 - 2026-09-03

DSH 0.1.1-rc.2 独立候选；rc6 使用 0.5.5-rc.3、alpha5 使用 0.5.7-alpha.3，不混装。

- native host/platform 升级目标为 0.1.0-rc.2，绑定 Rust 1.95 与新冻结补丁；公开发行必须核对本版本的真实执行和双构建凭据，不覆盖 rc.1。
- packed consumer 隔离空缓存；原 source lock 独立通过批准镜像预取后在新目录离线重放，保留原始 integrity。
- 安装文档统一三通道版本，明确 cold start、rc2 宿主补丁与热安装的不同边界；历史测试数据不冒用为新二进制验收。

## 0.5.6-rc.3 - 2026-09-03

匹配 DSH 0.1.1-rc.2；与 rc6 的 0.5.5-rc.2、alpha5 的 0.5.7-alpha.2 分开提供，不混装。

- GitHub #6 / ISSUE-033：公开 Actions 构建、类型、JUnit、政策来源、包消费者、校验和与按 tag 固定提交的 draft Release 流程；失败保留日志。
- 隔离安装器锁定完整公开 DSH 家族及本地 native 包，拒绝篡改、重复/未知制品、已有目录和符号链接。
- README 增加实际安装步骤、首次管理员 runtime 准备、三通道冷启动差异和热安装边界。
- 公开冷安装最终达到三通道六组件 ACTIVE、HTTP 200、退出 0；保留 rc6 缺失 peer/编译依赖等失败过程，不冒称最终脚本首次运行已无干预通过。
- 上游 native 受控开发 runner 与许可证材料闭环由 GitHub #5 跟踪；未通过完整门禁前不替换冻结 native 制品、不宣称完整生产认证。

## 0.5.6-rc.2 - 2026-09-03

GitHub 工程预发布；匹配 DSH 0.1.1-rc.2，不覆盖既有 tag 或线上部署，不发布 npm。
...
- ISSUE-028：普通 Web 新增模块图通过独立 9/9 及真实同页浏览器热安装验收，无刷新出现 Auto Review 并保存设置。
- GitHub #2：锁定仓库内桥接候选档案，修复公开 CI 依赖未发布 npm 包；真实全新安装与 214 项回归通过。
- GitHub #3：补齐公开仓库信息、DSH 补丁许可、预发布范围与安装文档；保留第三方权利及完整原生执行系统未完成项，不把候选版本标为生产认证。
