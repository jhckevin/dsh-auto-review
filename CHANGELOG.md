...
### 0.5.6-rc.5 — ISSUE-034 / GitHub #12

仅 DSH 0.1.1-rc.2 开发候选；未替换旧 Release 或线上服务。

- 拒绝熔断改为原生当前回合取消，不再退回人工审批；同步锁定计数、三层执行门禁、冻结首个熔断快照，隔离迟到/取消审查和下一回合。
- WebUI 原生 turnTail 显示持久的红色盾牌与斜线告警，正常完成和用户取消不误报；不向模型注入伪造消息。
- 提供显式加载的终端 notice adapter 与 terminal.patch.yml，清理 ANSI/控制字符并去重；不冒称固定 rc.2 存在完整 TUI renderer。
- 新增真实 DSH AgentLoop、并发取消、迟到批准、计数边界、UI 与终端生命周期回归；模型/reviewer使用确定性适配器，不冒称付费模型实测。
