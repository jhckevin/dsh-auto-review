...
function fakeAgent(id = 'session-auto-review', request = 'Run the exact diagnostic command if needed.') {
  ...
  } as unknown as Parameters<typeof Session.create>[2])
  ...
  return { agent, get appended() { return sessionEvents(session, start as unknown as number) } }
}
...
describe('native tools pipeline composition', () => {
  ...
  it.each(['approved', 'manual', 'unavailable'] as const)('never rejects escalation after reviewer %s without consulting answerers', async (outcome) => {
    ...
    expect(sessionEvents(agent.session).find(event => event.type === 'approval/decided')?.data).toMatchObject({ outcome: 'rejected' })
  })
